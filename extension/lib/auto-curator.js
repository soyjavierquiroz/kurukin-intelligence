...
(function(root){
  ...
  function create({storage,tabs,alarms,now=Date.now,instanceId=`auto-${Math.random().toString(36).slice(2)}`}={}){
    ...
    async function recoverDiscovery(state,channel,reason){
      ...
      try{
        // The content relay cancels MAIN-world work before the next run.  This
        // explicit stop prevents a stale controller or checkpoint promise from
        // treating the unchanged logical scan_id as already busy.
        if(tabs?.pause)await tabs.pause(state.tab_id);
        const current=active(state);if(!current)return state;
        Object.assign(current,{status:'recovering',recovery_state:'restoring_profile',updated_at:nowISO(now)});state.updated_at=nowISO(now);await save(state);
        const onTarget=await open(state,current,true);
        Object.assign(current,{status:'recovering',recovery_state:onTarget?'rehydrating':'restoring_profile',profile_matches_active_channel:onTarget===true,updated_at:nowISO(now)});state.updated_at=nowISO(now);await save(state);
      }catch{
        const current=active(state);if(current)Object.assign(current,{status:'error',last_error:'DIRECT_INTERNAL',recovery_state:'failed',updated_at:nowISO(now)});state.status='paused';state.updated_at=nowISO(now);await save(state);
      }
      return state;
    }
