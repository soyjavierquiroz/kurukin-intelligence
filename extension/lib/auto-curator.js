...
(function(root){
  ...
  function channelRecord(item,index,at){return {id:`channel-${index}-${item.channel}`,channel:item.channel,profile_url:item.profile_url,status:'pending',started_at:null,updated_at:at,last_error:null,processed_count:0,acquired_count:0,analysis_id:null,scan_id:null,checkpoint_saved_count:0,checkpoint_discovered_count:0,discovery_state:'idle',acquisition_state:'idle',acquisition_capacity_state:'normal',acquisition_cooldown_until:null,last_capacity_error_code:null,drain_state:'scanning',acquisition:acquisition()};}
  ...
  function transition(original,event,now=Date.now){
    ...
    switch(event.type){
      ...
      case 'SCANNING': update('scanning',{navigation_attempts:0,scan_target:event.scan_target==='full'||Number.isSafeInteger(event.scan_target)?event.scan_target:current?.scan_target||null,analysis_id:event.analysis_id||current?.analysis_id||null,scan_id:event.scan_id||current?.scan_id||null,resume_state:event.resume_state||current?.resume_state||null,scan_complete:false,discovery_state:'scanning',drain_state:'scanning'}); break;
      ...
      case 'PENDING': update(current?.status||'scanning',{pending_incremental:Number.isSafeInteger(event.incremental)?event.incremental:current?.pending_incremental||0,pending_final:Number.isSafeInteger(event.final)?event.final:current?.pending_final||0}); break;
