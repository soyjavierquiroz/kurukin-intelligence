...
(()=>{
  ...
  function clear(){active?.controller.abort();active=null;job?.controller.abort();job=null;completed=null;for(const settle of checkpointAcks.values())settle.reject(S.fail('DIRECT_CANCELLED'));checkpointAcks.clear();media.clear();}
  window.addEventListener('message',async event=>{
    ...
    if(type==='KURUKIN_SCAN_START'){
      ...
      try{const checkpoint=p=>new Promise((resolve,reject)=>{if(active!==scan)return reject(S.fail('DIRECT_CANCELLED'));checkpointAcks.set(p.checkpointNumber,{resolve,reject});send('SCAN_CHECKPOINT',scanId,p);});const result=await collector.scan({target:payload.target,resume:payload.resume||null,signal:scan.controller.signal,onProgress:p=>{if(active===scan)send('SCAN_PROGRESS',scanId,p);},onCheckpoint:checkpoint,onItem:(item,video)=>{if(active===scan)media.set(video.id,{videoId:video.id,duration:video.duration,candidates:A.candidates(item)});}});if(active===scan){if(!result.cancelled)completed={scanId,username:context.targetUsername()};else media.clear();send('SCAN_COMPLETE',scanId,result);}}catch(error){if(active===scan){clear();send('SCAN_ERROR',scanId,{code:S.safeError(error)});}}finally{if(active===scan)active=null;}return;
    }
