...
const checkpointMessage=value=>exact(value,['type','payload'])&&value.type==='KURUKIN_BACKEND_CHECKPOINT'&&!!value.payload&&typeof value.payload==='object'&&uuid(value.payload.analysis_id)&&uuid(value.payload.scan_id)&&Number.isSafeInteger(value.payload.checkpoint_number)&&Number.isSafeInteger(value.payload.checkpoint_count)&&Number.isSafeInteger(value.payload.discovered_count)&&Number.isSafeInteger(value.payload.target)&&Array.isArray(value.payload.videos)&&value.payload.videos.length>0&&value.payload.videos.length<=50&&value.payload.videos.every(video);
const nextBatchMessage=value=>exact(value,['type','analysisId'])&&value.type==='KURUKIN_BACKEND_NEXT_BATCH'&&uuid(value.analysisId);
