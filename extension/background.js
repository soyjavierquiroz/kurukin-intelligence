...
const nextBatchMessage=value=>exact(value,['type','analysisId','discoveryComplete','hasMore'])&&value.type==='KURUKIN_BACKEND_NEXT_BATCH'&&uuid(value.analysisId)&&typeof value.discoveryComplete==='boolean'&&typeof value.hasMore==='boolean';
...
const autoCommand=value=>!!value&&typeof value==='object'&&['KURUKIN_AUTO_QUEUE','KURUKIN_AUTO_PAUSE','KURUKIN_AUTO_RESUME','KURUKIN_AUTO_RETRY_CURRENT','KURUKIN_AUTO_RESET_CURRENT','KURUKIN_AUTO_STOP','KURUKIN_AUTO_CLEAR','KURUKIN_AUTO_REMOVE_PENDING','KURUKIN_AUTO_SKIP','KURUKIN_AUTO_GET'].includes(value.type)&&(!Object.hasOwn(value,'reason')||(value.type==='KURUKIN_AUTO_RESET_CURRENT'&&typeof value.reason==='string'&&value.reason.length>0&&value.reason.length<=128));

chrome.runtime.onMessage.addListener((message,sender,sendResponse)=>{
  if(AUTO_CURATOR_V1_ENABLED&&autoCommand(message)&&panelSender(sender)){
    (async()=>{try{const runner=curator(),tabId=await panelTab(sender);let value;if(message.type==='KURUKIN_AUTO_QUEUE'){if(!Number.isInteger(tabId))throw Error('AUTO_TIKTOK_TAB_REQUIRED');value=await runner.enqueue(message.channels,tabId);}else if(message.type==='KURUKIN_AUTO_PAUSE')value=await runner.pause();else if(message.type==='KURUKIN_AUTO_RESUME')value=await runner.resume();else if(message.type==='KURUKIN_AUTO_RETRY_CURRENT')value=await runner.retryCurrent();else if(message.type==='KURUKIN_AUTO_RESET_CURRENT')value=await runner.resetCurrent(message.reason||'manual_reset');else if(message.type==='KURUKIN_AUTO_STOP')value=await runner.stop();else if(message.type==='KURUKIN_AUTO_CLEAR')value=await runner.clear();else if(message.type==='KURUKIN_AUTO_REMOVE_PENDING'&&typeof message.channelId==='string')value=await runner.removePending(message.channelId);else if(message.type==='KURUKIN_AUTO_SKIP')value=await runner.skip();else value=await runner.load();sendResponse({ok:true,value});}catch{sendResponse({ok:false,error:'AUTO_CURATOR_ERROR'});}})();return true;
  }
