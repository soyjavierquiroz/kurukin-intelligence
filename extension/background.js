...
const AUTO_CURATOR_V1_ENABLED=true; // Local-development flag. Do not enable for a release without an operator dry run.
let auto=null;
const curator=()=>auto||(auto=globalThis.KurukinAutoCurator.create({
  ...
  tabs:{navigate:(tabId,url)=>Number.isInteger(tabId)?chrome.tabs.update(tabId,{url}):Promise.reject(Error('AUTO_TAB_MISSING')),run:(tabId,channel)=>Number.isInteger(tabId)?chrome.tabs.sendMessage(tabId,{type:'KURUKIN_AUTO_RUN',channel:{id:channel.id,channel:channel.channel,target:200}}).catch(()=>{}):Promise.resolve(),pause:tabId=>Number.isInteger(tabId)?chrome.tabs.sendMessage(tabId,{type:'KURUKIN_AUTO_PAUSE'}).catch(()=>{}):Promise.resolve()},
  alarms:{create:(name,info)=>chrome.alarms.create(name,info)},instanceId:'extension-service-worker'
}));
...
const autoCommand=value=>!!value&&typeof value==='object'&&['KURUKIN_AUTO_START','KURUKIN_AUTO_PAUSE','KURUKIN_AUTO_RESUME','KURUKIN_AUTO_STOP','KURUKIN_AUTO_SKIP','KURUKIN_AUTO_GET'].includes(value.type);

chrome.runtime.onMessage.addListener((message,sender,sendResponse)=>{
  if(AUTO_CURATOR_V1_ENABLED&&autoCommand(message)&&panelSender(sender)){
    (async()=>{try{const runner=curator(),tabId=await panelTab(sender);let value;if(message.type==='KURUKIN_AUTO_START'){if(!Number.isInteger(tabId))throw Error('AUTO_TIKTOK_TAB_REQUIRED');value=await runner.start(message.channels,tabId);}else if(message.type==='KURUKIN_AUTO_PAUSE')value=await runner.pause();else if(message.type==='KURUKIN_AUTO_RESUME')value=await runner.resume();else if(message.type==='KURUKIN_AUTO_STOP')value=await runner.stop();else if(message.type==='KURUKIN_AUTO_SKIP')value=await runner.skip();else value=await runner.load();sendResponse({ok:true,value});}catch{sendResponse({ok:false,error:'AUTO_CURATOR_ERROR'});}})();return true;
  }
  ...
  if(!tiktokSender(sender)||(!reserveMessage(message)&&!uploadMessage(message)&&!failureMessage(message))){sendResponse({ok:false,error:backend.safeError()});return false;}
  (async()=>{
    try{
      const value=message.type==='KURUKIN_BACKEND_RESERVE'?await backend.reserve(fetch,message.videos):message.type==='KURUKIN_BACKEND_FAILURE'?await backend.reportFailure(fetch,message.analysisId,message.tiktokId,message.code):await workerUpload.upload(fetch,message);
