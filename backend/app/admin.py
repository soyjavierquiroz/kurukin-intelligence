...
.field-help{{display:block;color:#64748b;font-size:.88rem;margin:4px 0 8px}}.choice-grid{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin:8px 0}}.choice{{display:flex;align-items:center;gap:8px;border:1px solid #b9c6d4;border-radius:8px;padding:10px;min-height:44px;cursor:pointer}}.choice input{{accent-color:#1659b7}}.prepared-context{{margin-top:14px}}.prepared-summary{{display:grid;gap:4px;margin:12px 0}}.hormozi-handoff{{margin-top:18px;padding-top:14px;border-top:1px solid #dce3eb}}@media(max-width:650px){{.choice-grid{{grid-template-columns:1fr}}.choice{{min-height:48px}}}}
</style></head><body><main><header><h1><a href="/admin/research">Kurukin</a></h1><span class="muted">KURUKIN PRODUCT LITE v1.2 · SCI v1 · Build {_e(_build_marker())}</span></header>{journey}{content}</main></body></html>''')
...
def _creative_context_from_form(business: str = '', offer: str = '', audience: str = '', objective: str = '',
    ...
    if normalized['cta_preference'] == 'Otro' and normalized['cta_custom']:
        normalized['cta_preference'] = normalized['cta_custom']
    del normalized['cta_custom']
    return normalized
...
def creative_super_prompt(context: dict[str, str]) -> str:
    return f'''You previously analyzed this competitor channel for me.

Kurukin has now processed that analysis and combined it with information about my own business.

Use BOTH:
- the context you already have from this conversation
- the attached kurukin-creative-context.md

If there is any conflict, treat the attached Kurukin context as the current source of truth for my business and strategy.

Study the attached Kurukin Creative Context before proposing creative work. Understand the channel intelligence, its evidence, and the user's business context.
...
def download_creative_context(channel_id: uuid.UUID, business: str = Form(''), offer: str = Form(''),
                              ...
                              tone: str = Form(''), cta_preference: str = Form(''), cta_custom: str = Form(''), restrictions: str = Form(''),
    ...
    context = _creative_context_from_form(business, offer, audience, objective, market, tone, cta_preference,
                                          restrictions, additional_context, cta_custom)
...
def creative_super_prompt_text(channel_id: uuid.UUID, business: str = Form(''), offer: str = Form(''),
    ...
    return PlainTextResponse(creative_super_prompt(_creative_context_from_form(
        business, offer, audience, objective, market, tone, cta_preference, restrictions, additional_context, cta_custom)))


@router.get('/research/channels/{channel_id}/intelligence', response_class=HTMLResponse)
def channel_intelligence_page(channel_id: uuid.UUID, imported: bool = False,
                              _auth: None = Depends(require_admin), db: Session = Depends(get_db)):
...
<section><p class="eyebrow">CANAL</p><div class="stat-grid"><div class="stat"><b>{data['total_videos']}</b>videos encontrados</div><div class="stat"><b>{data['priority_transcripts']}</b>transcripciones disponibles</div></div></section>'''
    ...
    import_success = '''<div class="card ok"><p><b>Inteligencia importada correctamente.</b></p><p>Si analizaste este canal con Alex Hormozi, mantén abierta esa conversación. La volveremos a usar para crear contenido con todo el contexto acumulado.</p></div>''' if imported else ''
    ...
    content = f'''{channel_header}{import_success}{update_notice}
...
<section id="usar-inteligencia" class="card"><p class="eyebrow">3. USAR ESTA INTELIGENCIA</p><h2>Prepara tu contexto</h2><p class="muted">Cuéntanos lo esencial y usa la inteligencia del canal para continuar en el mismo chat de Hormozi.</p><form id="creative-context-form"><label>¿Qué vendes?<br><span class="field-help">Describe brevemente tu negocio y tus principales productos o servicios.</span><textarea name="business" required maxlength="4000" style="min-height:85px" placeholder="Mística: sahumerios, aceites esenciales y productos rituales para bienestar y armonización del hogar."></textarea></label><label>¿A quién quieres vender?<br><span class="field-help">Describe a tu cliente ideal con palabras simples.</span><textarea name="audience" required maxlength="4000" style="min-height:85px" placeholder="Mujeres de 35–55 años interesadas en espiritualidad, autocuidado y bienestar del hogar."></textarea></label><fieldset><legend>¿Qué quieres conseguir?</legend><div class="choice-grid"><label class="choice"><input type="radio" name="objective" value="Ventas" required>Ventas</label><label class="choice"><input type="radio" name="objective" value="Leads">Leads</label><label class="choice"><input type="radio" name="objective" value="Autoridad">Autoridad</label><label class="choice"><input type="radio" name="objective" value="Audiencia">Audiencia</label></div></fieldset><label>¿Cómo quieres convertir esa atención?<br><select id="cta-preference" name="cta_preference" required><option value="" selected disabled>Elige una opción</option><option>WhatsApp</option><option>Mensaje privado</option><option>Link en bio</option><option>Comentarios</option><option>Otro</option></select></label><label id="cta-custom-field" hidden>Especifica cómo quieres convertir la atención<br><input name="cta_custom" maxlength="4000" placeholder="Por ejemplo: llamada de diagnóstico"></label><details id="advanced-context"><summary><b>Personalizar más</b></summary><div class="step-body"><label>Detalles adicionales de tu oferta<br><textarea name="offer" maxlength="4000" style="min-height:70px"></textarea></label><label>Mercado<br><input name="market" maxlength="4000" placeholder="Por ejemplo: México"></label><label>Tono / estilo<br><input name="tone" maxlength="4000" placeholder="Por ejemplo: cercano y claro"></label><label>Restricciones<br><textarea name="restrictions" maxlength="4000" style="min-height:70px"></textarea></label><label>Contexto adicional<br><textarea name="additional_context" maxlength="4000" style="min-height:70px"></textarea></label></div></details><div class="actions"><button id="prepare-creative-context" type="button">Preparar mi contexto creativo</button></div><div id="creative-context-ready" class="prepared-context" hidden><div class="prepared-summary">✓ Inteligencia del canal<br>✓ Evidencia representativa<br>✓ Tu negocio<br>✓ Tu audiencia<br>✓ Tu objetivo<br>✓ Tu CTA</div><div class="actions"><button id="download-creative-context" type="button">Descargar contexto</button></div><div id="hormozi-handoff" class="hormozi-handoff"><p><b>Vuelve al mismo chat de Alex Hormozi que utilizaste para analizar este canal.</b></p><ol><li>Vuelve al chat anterior de Hormozi.</li><li>Adjunta <code>kurukin-creative-context.md</code>.</li><li>Pega el Super Prompt.</li><li>Continúa trabajando libremente con Hormozi.</li></ol><div class="actions"><button id="copy-super-prompt" type="button">Copiar Super Prompt</button></div><p class="muted">¿Ya no tienes abierta la conversación anterior?</p><div class="actions"><button id="open-new-hormozi" type="button" class="secondary">Abrir nuevo Alex Hormozi GPT</button></div></div></div></form><p id="creative-context-status" class="muted" aria-live="polite"></p></section>
...
<script>(function(){{const form=document.getElementById('creative-context-form'),status=document.getElementById('creative-context-status'),ready=document.getElementById('creative-context-ready'),cta=document.getElementById('cta-preference'),ctaCustom=document.querySelector('[name="cta_custom"]'),ctaCustomField=document.getElementById('cta-custom-field'),base='/admin/research/channels/{channel_id}/intelligence',gptUrl='{HORMOZI_GPT_URL}';let contextBlob=null;function body(){{return new FormData(form)}}function updateCtaCustom(){{const other=cta.value==='Otro';ctaCustomField.hidden=!other;ctaCustom.required=other}}cta.addEventListener('change',updateCtaCustom);async function prepare(){{if(!form.reportValidity())return;const r=await fetch(base+'/creative-context.md',{{method:'POST',body:body()}});if(!r.ok)throw new Error('No se pudo preparar tu contexto.');contextBlob=await r.blob();ready.hidden=false;status.textContent='Tu contexto está listo para descargar y continuar en Hormozi.'}}function downloadPrepared(){{if(!contextBlob)throw new Error('Primero prepara tu contexto.');const url=URL.createObjectURL(contextBlob),a=document.createElement('a');a.href=url;a.download='kurukin-creative-context.md';document.body.appendChild(a);a.click();a.remove();URL.revokeObjectURL(url)}}document.getElementById('prepare-creative-context').addEventListener('click',async()=>{{try{{await prepare()}}catch(error){{status.textContent=error.message}}}});document.getElementById('download-creative-context').addEventListener('click',()=>{{try{{downloadPrepared();status.textContent='Contexto descargado.'}}catch(error){{status.textContent=error.message}}}});document.getElementById('copy-super-prompt').addEventListener('click',async()=>{{try{{const r=await fetch(base+'/creative-super-prompt',{{method:'POST',body:body()}});if(!r.ok)throw new Error();await navigator.clipboard.writeText(await r.text());status.textContent='Super Prompt copiado. Vuelve al chat de Hormozi para pegarlo.'}}catch(error){{status.textContent='No se pudo copiar el Super Prompt automáticamente.'}}}});document.getElementById('open-new-hormozi').addEventListener('click',()=>{{window.open(gptUrl,'_blank','noopener')}});}})();</script>'''
